...
public class RoundedCylinder extends Primitive {
	 ...
	 * The resolution of the tessellation can be controlled with {@code numSlices}.
	 *
	 * @param startRadius
	...
	public List<Polygon> toPolygons() {
		...
		for (int i = 0; i < numCornerSlices; i++) {
			double radIncs = startRadius - cornerRadius
					+ Math.sin(Math.PI / 2 * ((double) i / (double) numCornerSlices)) * cornerRadius;
			double radInce = endRadius - cornerRadius
					+ Math.sin(Math.PI / 2 * ((double) i / (double) numCornerSlices)) * cornerRadius;

			double heightInc = (Math.cos(Math.PI / 2 * ((double) i / (double) (numCornerSlices))) * cornerRadius);
			cylParts.add(new Cylinder(radIncs, // Radius at the bottom

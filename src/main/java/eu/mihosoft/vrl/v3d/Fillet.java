...
public class Fillet extends Primitive {
  ...
  public static CSG outerFillet(CSG base, double rad) throws InvalidNormalException, TooFewPointsException, PointsColinearException, PointsNotCoplainer {
  ...
  public static CSG outerFillet(List<Polygon> polys, double rad) {
    ...
    for (Polygon p : polys) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (nextNext == size)
          nextNext = 0;
        Vector3d position0 = p.get(i).pos;
        Vector3d position1 = p.get(next).pos;
        Vector3d position2 = p.get(nextNext).pos;

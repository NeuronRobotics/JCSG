 ...
 * Algorithm for Convex Hulls'' (ACM Transactions on Mathematical Software, Vol.
 * 22, No. 4, December 1996), and has a complexity of O(n log(n)) with respect
 * to the number of points. A well-known C implementation of Quickhull that
 * works for arbitrary dimensions is provided by <a
 ...
 *
 * // x y z coordinates of 6 points Point3d[] points = new Point3d[] { new
 * Point3d (0.0, 0.0, 0.0), new Point3d (1.0, 0.5, 0.0), new Point3d (2.0, 0.0,
 * 0.0), new Point3d (0.5, 0.5, 0.5), new Point3d (0.0, 0.0, 2.0), new Point3d
 * (0.1, 0.2, 0.3), new Point3d (0.0, 2.0, 0.0), };
 *
 * QuickHull3D hull = new QuickHull3D(); hull.build (points);
 *
 * //com.neuronrobotics.sdk.common.Log.error ("Vertices:"); Point3d[] vertices =
 * hull.getVertices(); for (int i = 0; i < vertices.length; i++) { Point3d pnt =
 * vertices[i]; //com.neuronrobotics.sdk.common.Log.error (pnt.x + " " + pnt.y +
 * " " + pnt.z); }
 *
 * //com.neuronrobotics.sdk.common.Log.error ("Faces:"); int[][] faceIndices =
 * hull.getFaces(); for (int i = 0; i < faceIndices.length; i++) { for (int k =
 * 0; k < faceIndices[i].length; k++) { System.out.print (faceIndices[i][k] + "
 ...
 * convex if the centroid of each adjacent plane is clearly below the plane of
 * the other face. The centroid is considered below a plane if its distance to
 * the plane is less than the negative of a {@link #getDistanceTolerance()
 * distance tolerance}. This tolerance represents the smallest distance that can
 * be reliably computed within the available numeric precision. It is normally
 ...
 * with triangulated output from <a href=http://www.qhull.org>qhull ).
 *
...
class QuickHull3D {
	 ...
	 * =#distTol>numerical imprecision . Normally, this tolerance is computed
	 * automatically for each set of input points, but it can be set explicitly by
	 * the application.
	 *
	 ...
	 *
	 * @param coords
	 *            x, y, and z coordinates of each input point. The length of this
	...
	private void printQhullErrors(Process proc) throws IOException {
		...
		if (wrote) { // com.neuronrobotics.sdk.common.Log.error("");
		}
	}
	...
	protected void setFromQhull(double[] coords, int nump, boolean triangulate) {
		...
		try {
			...
			if (stok.ttype != StreamTokenizer.TT_NUMBER) { // com.neuronrobotics.sdk.common.Log.error ("Expecting number
															// of faces");
				System.exit(1);
			}
			...
			for (int i = 0; i < numf; i++) {
				...
				while (stok.nextToken() != StreamTokenizer.TT_EOL) {
					if (stok.ttype != StreamTokenizer.TT_NUMBER) { // com.neuronrobotics.sdk.common.Log.error
	 ...
	 * @throws IllegalArgumentException
	 *             the number of input points is less than four or greater than 1/3
	 *             the length of <code>coords</code>, or the points appear to be
	 ...
	 *            number of input points
	 * @throws IllegalArgumentException
	 *             the number of input points is less than four or greater then the
	 *             length of <code>points</code>, or the points appear to be
	 *             coincident, colinear, or coplanar.
	 */
	 ...
	 * to be non-convex (this same limitation is present in
	...
	protected void createInitialSimplex() throws IllegalArgumentException {
		...
		if (debug) { // com.neuronrobotics.sdk.common.Log.error ("initial vertices:");
						// com.neuronrobotics.sdk.common.Log.error (vtx[0].index + ": " + vtx[0].pnt);
						// com.neuronrobotics.sdk.common.Log.error (vtx[1].index + ": " + vtx[1].pnt);
						// com.neuronrobotics.sdk.common.Log.error (vtx[2].index + ": " + vtx[2].pnt);
						// com.neuronrobotics.sdk.common.Log.error (vtx[3].index + ": " + vtx[3].pnt);
		}
	...
	protected void resolveUnclaimedPoints(FaceList newFaces) {
		...
		for (Vertex vtx = vtxNext; vtx != null; vtx = vtxNext) {
			...
			if (maxFace != null) {
				...
				if (debug && vtx.index == findIndex) { // com.neuronrobotics.sdk.common.Log.error (findIndex + " CLAIMED
														// BY " + maxFace.getVertexString());
				}
			} else {
				if (debug && vtx.index == findIndex) { // com.neuronrobotics.sdk.common.Log.error (findIndex + "
														// DISCARDED");
				}
			}
		}
	...
	private boolean doAdjacentMerge(Face face, int mergeType) {
		...
		do {
			...
			if (merge) {
				if (debug) { // com.neuronrobotics.sdk.common.Log.error (" merging " + face.getVertexString()
				...
				if (debug) { // com.neuronrobotics.sdk.common.Log.error (" result: " +
	...
	protected void calculateHorizon(Point3d eyePnt, HalfEdge edge0, Face face, Vector horizon) {
		...
		if (debug) { // com.neuronrobotics.sdk.common.Log.error (" visiting face " +
						// face.getVertexString());
		}
		HalfEdge edge;
		if (edge0 == null) {
		...
		do {
			...
			if (oppFace.mark == Face.VISIBLE) {
				if (oppFace.distanceToPlane(eyePnt) > tolerance) {
					...
					horizon.add(edge);
					if (debug) { // com.neuronrobotics.sdk.common.Log.error (" adding horizon edge " +
	...
	protected void addNewFaces(FaceList newFaces, Vertex eyeVtx, Vector horizon) {
		...
		for (Iterator it = horizon.iterator(); it.hasNext();) {
			try {
				...
				if (debug) {
					// com.neuronrobotics.sdk.common.Log.error("new face: " +
	...
	protected void addPointToHull(Vertex eyeVtx) {
		...
		if (debug) { // com.neuronrobotics.sdk.common.Log.error ("Adding point: " + eyeVtx.index);
			// com.neuronrobotics.sdk.common.Log.error (" which is " +
	...
	protected void buildHull() {
		...
		while ((eyeVtx = nextPointToAdd()) != null) {
			...
			cnt++;
			if (debug) { // com.neuronrobotics.sdk.common.Log.error ("iteration " + cnt + " done");
			}
		}
		reindexFacesAndVertices();
		if (debug) { // com.neuronrobotics.sdk.common.Log.error ("hull done");
		}
	}
	 ...
	 * performed using the distance tolerance tol . Faces are considered non-convex
	 * if any edge is non-convex, and an edge is non-convex if the centroid of
	 * either adjoining face is more than tol above the plane of the other face.
	 * Similarly, a point is considered outside a face if its distance to that
	 * face's plane is more than 10 times tol .
	 *
